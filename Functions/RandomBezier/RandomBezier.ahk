...
; URL (+info):  --------------------
;
; Last Update:  04/October/2012 02:00h BRT
;
...
RandomBezier( X0, Y0, Xf, Yf, O="" ) {
  Time := RegExMatch(O,"i)T(\d+)",M)&&(M1>0)? M1: 200, R:=InStr(O,"Rel",0)

  /*
  Max := RegExMatch(O,"i)Max(\d+)",M) ? ( (M1<2) ? 2 : ((M1>19) ? 19 : M1) ) : 5
  Min := RegExMatch(O,"i)Min(\d+)",M) ? ( (M1<2) ? 2 : ((M1>19) ? 19 : M1) ) : 2
  Random, N, %Min%, %Max%
  */
